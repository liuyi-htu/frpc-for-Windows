...
func (s *svcProgram) runFrpcLoop() {
	for {
		...
		// Keep the log destination fixed even when the raw config was edited
		// outside the web console.
		if config := readText(s.paths.Config); config != "" {
			fixed := normalizeConfigText(config)
			if fixed != config {
				if err := os.WriteFile(s.paths.Config, []byte(fixed), 0644); err != nil {
					logLine(s.paths, "service.log", "stopped: failed to enforce fixed log path: "+err.Error())
					if !sleepOrStop(s.stop, 5*time.Second) {
						return
					}
					continue
...
func (w *webApp) apiBasic(rw http.ResponseWriter, r *http.Request) {
	...
	writeJSON(rw, map[string]any{
		...
		"authMode":   basicAuthMode(text),
		"token":      basicTokenValue(text),
		"logLevel":   defaultString(tomlValue(text, "log.level"), "info"),
		"logMaxDays": defaultString(tomlValue(text, "log.maxDays"), "7"),
	})
}
...
func normalizeConfigText(text string) string {
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(trimmed, "log.to") {
			continue
		}
		if strings.HasPrefix(trimmed, "log.level") {
			hasLogLevel = true
		}
		if strings.HasPrefix(trimmed, "log.maxDays") {
			hasLogMaxDays = true
		}
		if strings.Contains(line, "remotePort 6000") && (strings.Contains(line, "\u7f02") || strings.Contains(line, "\u95c2")) {
			line = fixedProxyComment
		}
		if !insertedLogs && strings.HasPrefix(trimmed, "[[proxies]]") {
			filtered = append(filtered, "log.to = \"./logs/frpc.log\"")
			if !hasLogLevel {
				filtered = append(filtered, "log.level = \"info\"")
			}
			if !hasLogMaxDays {
				filtered = append(filtered, "log.maxDays = 7")
			}
...
func (w *webApp) apiSave(rw http.ResponseWriter, r *http.Request) {
	...
	if err := os.WriteFile(w.paths.Config, []byte(normalizeConfigText(body.Content)), 0644); err != nil {
...
func basicAuthMode(text string) string {
	if tomlValue(text, "metadatas.token") != "" {
		return "metadatas"
	}
	return "auth"
}

func basicTokenValue(text string) string {
	if token := tomlValue(text, "metadatas.token"); token != "" {
		return token
	}
	return tomlValue(text, "auth.token")
}
...
func saveBasicConfig(p paths, b basicConfig) error {
	...
	b.AuthMode = strings.TrimSpace(b.AuthMode)
	b.Token = strings.TrimSpace(b.Token)
	if b.AuthMode == "" {
		b.AuthMode = "metadatas"
	}
	...
	if b.AuthMode != "metadatas" && b.AuthMode != "auth" {
		return errors.New("authMode must be metadatas or auth")
	}
	...
	if b.Token != "" {
		if b.AuthMode == "metadatas" {
			out = append(out, `metadatas.token = "`+tomlEscape(b.Token)+`"`, "")
		} else {
